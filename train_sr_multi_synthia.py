...
from trainer_sr_multi import AD_Trainer
...
def get_arguments():
    ...
    parser.add_argument("--lambda-adv-target2", type=float, default=LAMBDA_ADV_TARGET2,
                        help="lambda_adv for adversarial training.")
    parser.add_argument("--lambda-kl-target1", type=float, default=LAMBDA_KL_TARGET1,
                        help="lambda_me for minimize kl loss on target.")
    parser.add_argument("--lambda-kl-target2", type=float, default=LAMBDA_KL_TARGET2,
                        help="lambda_me for minimize kl loss on target.")
    parser.add_argument("--epsilon1", type=float, default=EPSILON1,
                        help="hyper-parameter for attack noise")
    parser.add_argument("--epsilon2", type=float, default=EPSILON2,
                        help="hyper-parameter for attack noise")
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        loss_D_value1 = 0
        loss_kl_target_value1 = 0

        loss_seg_value2 = 0
        loss_adv_target_value2 = 0
        loss_D_value2 = 0
        loss_kl_target_value2 = 0
        ...
        for sub_i in range(args.iter_size):
            ...
            with Timer("Elapsed time in update: %f"):
                loss_seg1, loss_seg2, loss_adv_target1, loss_adv_target2, loss_kl1, loss_kl2, pred1, \
                pred2, pred_target1, pred_target2, pred_target_noise1, pred_target_noise2, val_loss \
                    = Trainer.gen_update(images, images_t, labels, labels_t, i_iter)
                ...
                loss_kl_target_value1 += loss_kl1 / args.iter_size
                loss_kl_target_value2 += loss_kl2 / args.iter_size
        ...
        del pred1, pred2, pred_target1, pred_target2, pred_target_noise1, pred_target_noise2
        ...
        if args.tensorboard:
            scalar_info = {
                ...
                'loss_kl_target1': loss_kl_target_value1,
                'loss_kl_target2': loss_kl_target_value2,
        ...
        print(
            '\033[1m iter = %8d/%8d \033[0m loss_seg1 = %.3f loss_seg2 = %.3f loss_kl1 = %.3f  loss_kl2 = %.3f loss_adv1 = %.3f, loss_adv2 = %.3f loss_D1 = %.3f loss_D2 = %.3f, val_loss=%.3f' % (
            i_iter, args.num_steps, loss_seg_value1, loss_seg_value2, loss_kl_target_value1, loss_kl_target_value2, loss_adv_target_value1,
            loss_adv_target_value2, loss_D_value1, loss_D_value2, val_loss))

        # clear loss
        del loss_seg1, loss_seg2, loss_adv_target1, loss_adv_target2, loss_kl1, loss_kl2, val_loss

        if i_iter >= args.num_steps_stop - 1:
            print('save model ...')
            torch.save(Trainer.G.state_dict(), osp.join(args.snapshot_dir, 'synthia_' + str(args.num_steps_stop) + '.pth'))
            torch.save(Trainer.D1.state_dict(),
                       osp.join(args.snapshot_dir, 'synthia_' + str(args.num_steps_stop) + '_D1.pth'))
            torch.save(Trainer.D2.state_dict(),
                       osp.join(args.snapshot_dir, 'synthia_' + str(args.num_steps_stop) + '_D2.pth'))
            break

        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            print('taking snapshot ...')
            torch.save(Trainer.G.state_dict(), osp.join(args.snapshot_dir, 'synthia_' + str(i_iter) + '.pth'))
            torch.save(Trainer.D1.state_dict(), osp.join(args.snapshot_dir, 'synthia_' + str(i_iter) + '_D1.pth'))
            torch.save(Trainer.D2.state_dict(), osp.join(args.snapshot_dir, 'synthia_' + str(i_iter) + '_D2.pth'))
