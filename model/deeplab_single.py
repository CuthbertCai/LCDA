...
class Classifier_Module(nn.Module):
    def __init__(self, inplanes, dilation_series, padding_series, num_classes, norm_style='bn', droprate=0.1,
                 use_se=False):
        ...
        if use_se:
            self.bottleneck = nn.Sequential(*[SEBlock(256 * (len(dilation_series) + 1)),
                                              nn.Conv2d(256 * (len(dilation_series) + 1), 512, kernel_size=3, stride=1,
                                                        padding=1, dilation=1, bias=True),
                                              NormLayer(512, norm_style)])
        else:
            self.bottleneck = nn.Sequential(*[
                nn.Conv2d(256 * (len(dilation_series) + 1), 512, kernel_size=3, stride=1, padding=1, dilation=1,
                          bias=True),
                NormLayer(512, norm_style)])
...
class ResNetSingle(nn.Module):
    def __init__(self, block, layers, num_classes, use_se=False, train_bn=False, norm_style='bn', droprate=0.1):
        ...
        self.classifier = self.layer5
        # self.layer5 = self._make_pred_layer(Classifier_Module, 1024, [6, 12, 18, 24], [6, 12, 18, 24], num_classes,
        #                                     norm_style, droprate, use_se)
        # self.layer6 = self._make_pred_layer(Classifier_Module, 1024 + 2048, [6, 12, 18, 24], [6, 12, 18, 24],
        #                                     num_classes, norm_style, droprate, use_se)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, dilation=dilation, downsample=downsample, train_bn=self.train_bn))
    ...
    def _make_pred_layer(self, block, inplanes, dilation_series, padding_series, num_classes, norm_style, droprate,
                         use_se):
...
def DeeplabSingle(num_classes=21, use_se=False, train_bn=False, norm_style='bn', droprate=0.1):
    model = ResNetSingle(Bottleneck, [3, 4, 23, 3], num_classes, use_se=use_se, train_bn=train_bn, norm_style=norm_style,
                         droprate=droprate)
    return model
