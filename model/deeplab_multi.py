...
def DeeplabMulti(num_classes=21, use_se = False, train_bn = False, norm_style = 'bn', droprate = 0.1, arch='resnet101'):
    if arch == 'resnet101':
        ...
        model = ResNetMulti(Bottleneck, [3, 4, 6, 3], num_classes, use_se = use_se, train_bn = train_bn, norm_style = norm_style, droprate = droprate)
    else:
        raise Exception('Please choose a correct arch.')
    return model
