...
class AD_Trainer(nn.Module):
    def __init__(self, args):
        ...
        if args.model == 'DeepLabMulti':
            ...
            if args.restore_from[:4] == 'http':
                saved_state_dict = model_zoo.load_url(args.restore_from, map_location=torch.device('cpu'))
            else:
    ...
    def gen_update(self, images, images_t, labels, labels_t, i_iter):
        ...
        # target
        self.gen_opt.zero_grad()
        loss_adv_target1 = torch.tensor(0.0)
        loss_adv_target2 = torch.tensor(0.0)
        if self.lambda_adv_target1 > 0 or self.lambda_adv_target2 > 0:
